...
void AIPageContentAgent::DumpSkeletonNative() {
  ...
  HTMLElement* root = document->body();
  if (!root)
    return;

  base::Value::List sections;
  ...
  std::function<void(Node*)> walk = [&](Node* node) {
    ...
    if (node->IsElementNode()) {
      ...
      bool is_section = element->HasTagName(html_names::kSectionTag);
      const AtomicString& id = element->GetIdAttribute();
      if (!is_section) {
        if (id == AtomicString("hero") || id == AtomicString("pricing") ||
            id == AtomicString("features"))
          is_section = true;
      }
      ...
      if (is_section) {
        ...
        if (rect) {
          ...
          entry.Set("id", id.Utf8());
          entry.Set("x_min", rect->left());
          entry.Set("y_min", rect->top());
          entry.Set("x_max", rect->right());
          entry.Set("y_max", rect->bottom());
  ...
  root_dict.Set("sections", std::move(sections));

  std::string json;
  base::JSONWriter::Write(root_dict, &json);
  LOG(INFO) << "[BAI_SKELETON] " << json;
}
...
void AIPageContentAgent::ContentBuilder::AddNodeGeometry(
  ...
  if (attributes.dom_node_id.has_value() && !geometry.visible_bounding_box.IsEmpty()) {
    ...
    if (it != agent_->previous_node_geometries_.end()) {
      const gfx::Rect& prev_rect = it->second.first;
      const base::TimeTicks& prev_time = it->second.second;
