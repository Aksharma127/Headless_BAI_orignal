...
This bridge converts noisy stderr/stdout output from headless Chrome into clean
JSON.

- `extract_skeleton.py` reads stdin by default, or `--input <file>` for manual
  debugging.
...
4. In cloud mode, Python captures Chromium output directly with
   `subprocess.PIPE`.
5. The normalized result is written to Redis as `skeleton:{url_hash}`.
6. For local manual debugging, `extract_skeleton.py` can still write
   `clean_layout.json`.
